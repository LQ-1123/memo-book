"""从品牌图（app/static/icons/logo.png，Memo Book 螺旋丝带）合成 PWA 与桌面图标。

品牌图直接缩放放置，不重绘：白色圆角底（标准版）/ 全出血白底（maskable 与
apple-touch，后者由 iOS 自切圆角）。maskable 图形收进中心 ~80% 安全区。
...
LOGO = OUT / "logo.png"
BG = (255, 255, 255, 255)   # 品牌图即白底


def _logo_square() -> Image.Image:
    """品牌图补成正方形（居中，白底），保持构图不变形；黑角转透明。"""
